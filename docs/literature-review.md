...
**Exuvia— Multi-modal edge-AI sensors for proactive *Aedes aegypti* breeding-site detection.**

This document catalogues the prior work most relevant to Exuvia's design, grouped by topic. Each entry includes the citation, a one-paragraph summary, and how the work relates to (or differs from) Exuvia.
...
*Relevance to Exuvia:* primary training data source. The HumBug dataset is the foundation of Exuvia's acoustic classifier training.
...
*Relevance to Exuvia:* informs the choice of mel-spectrogram + 1D-CNN architecture and validates that noise-robust detection on field audio is feasible.
...
The closest published prior work to a naive version of Exuvia's acoustic stage. Trained a TinyML classifier on Edge Impulse using public wingbeat data; deployed to Arduino Nano 33 BLE Sense, Arduino Portenta H7, and Seeed Wio Terminal with LoRaWAN. Achieved 94% test-set accuracy distinguishing *Ae. aegypti* and *Ae. albopictus* from other species and background.

*Relevance to Exuvia:* directly establishes that microcontroller-class acoustic Aedes classification is feasible. **However, this work does not include any environmental signal**; it treats acoustic detection as the sole modality. Exuvia's contribution is the architectural inversion of using environmental signals as the primary trigger.
...
*Relevance to Exuvia:* informs lightweight 1D-CNN architecture choice. Demonstrates that very small (~tens of KB) INT8 models can perform stable classification on microcontroller-class hardware.
...
*Relevance to Exuvia:* informs the data-augmentation strategy (training with realistic background noise injection). Confirms that the field-noise robustness problem is acknowledged in the literature but not yet solved for microcontroller deployment.
...
*Relevance to Exuvia:* alternative sensing modality considered but rejected for the Exuvia use case — optical sensors require trap-coupling (mosquito flying through the sensor aperture), which is incompatible with the ambient breeding-site placement Exuvia targets.
...
*Relevance to Exuvia:* the closest commercial benchmark. **VECTRACK is trap-coupled, single-modal (acoustic only), and high-cost per unit.** Exuvia's contribution is to operate ambient (not trap-coupled), multi-modal, and at an order-of-magnitude lower per-unit cost.
...
Combines a traditional ovitrap with embedded electronics (Edge Impulse + FOMO object-detection) to automate counting of *Aedes aegypti* eggs. Tested in Argentina. The closest prior art to Exuvia's *concept* — IoT + edge ML applied to source-reduction-relevant biological signals.

*Relevance to Exuvia:* validates the conceptual direction (edge ML applied to upstream biological signals rather than adult counts). Differs from Exuvia in that it (a) targets egg-counting via vision, not breeding-site environmental sensing, (b) still requires deploying a managed ovitrap rather than instrumenting existing infrastructure (drains, planters).
...
*Relevance to Exuvia:* tangential — establishes that the wingbeat frequency profile of *Ae. aegypti* is well-characterised enough for engineered interaction. Not a direct part of Exuvia's architecture but informs future expansion paths.
...
*Relevance to Exuvia:* most conceptually aligned existing system — the LOCOMOBIS authors explicitly identify the value of combining acoustic detection with breeding-site data, but pursue it via aerial imagery rather than in-situ environmental sensing. The literature gap Exuvia fills is between LOCOMOBIS's acoustic-plus-imagery approach and a true site-resident multi-modal sensor.
...
*Relevance to Exuvia:* establishes that Singapore's residents are receptive to novel vector-control technology, which is a non-trivial enabler for Exuvia's eventual operational deployment.
...
*Relevance to Exuvia:* defines the existing surveillance baseline that Exuvia complements. Exuvia's per-site alerts can be quantitatively compared against the GAI at the same location for validation purposes.
...
*Relevance to Exuvia:* the primary source for the "real-world problem" framing in the grant proposal. Direct evidence of the scale of dengue burden in Singapore.
...
*Relevance to Exuvia:* describes the surveillance backbone Exuvia interfaces with, and the spatial heterogeneity in *Ae. aegypti* distribution within HDB blocks (lower floors significantly more than higher floors). Informs candidate site selection for Phase 3.
...
*Relevance to Exuvia:* validates the value of larval-stage signals (which Exuvia's optional hydrophone targets) as complementary to adult surveillance.
...
*Relevance to Exuvia:* defines the upstream-vs-downstream rationale. Adult abundance correlates with risk, but is downstream of breeding events; Exuvia's per-site alerts target the upstream signal.
...
*Relevance to Exuvia:* describes the current state-of-the-art predictive layer in NEA's programme. Exuvia's per-site alerts could in principle feed into future iterations of such models as a higher-resolution input feature.
...
*Relevance to Exuvia:* informs realistic performance expectations for the Exuvia acoustic stage on ESP32-S3.

### E2. David et al. (2021) — TensorFlow Lite Micro: Embedded machine learning for TinyML systems
*Proceedings of MLSys 2021*.

The reference paper for TensorFlow Lite Micro, the runtime targeted by Exuvia's on-device inference.

*Relevance to Exuvia:* tooling foundation for the firmware ML inference stage.

### E3. Edge Impulse documentation and case studies (continuously updated).

Edge Impulse is a major TinyML MLOps platform widely used in the mosquito-acoustic literature (Altayeb 2022, Kimutai 2023, Pruzzo 2024). Provides the workflow Exuvia's ML pipeline uses for data → preprocessing → model → quantization → deployment.

*Relevance to Exuvia:* primary platform for the ML pipeline. Edge Impulse's first-class ESP32-S3 support is one of the reasons for the choice of that MCU.
...
*Relevance to Exuvia:* informs the architectural pattern of using a cheap environmental signal to gate the expensive acoustic inference, which is the core engineering contribution of Exuvia.
...
*Relevance to Exuvia:* directly applicable to the power-budget calculation in `/hardware/power-budget.md`.

---

## G. Open questions in the literature that Exuvia can contribute to

Beyond filling the specific design gap that motivates the project, a successful Exuvia field study would generate evidence on several open questions in the surveillance literature:
...
Exuvia's field data, if released as an open dataset, would meaningfully contribute to all three of these.
